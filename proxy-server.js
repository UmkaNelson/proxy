...
app.post('/proxy/gemini-vision', async (req, res) => {
    ...
    try {
        ...
        // Исправленный промпт без конфликтующих обратных кавычек
        const prompt = `ВНИМАНИЕ: Ты должен отвечать ТОЛЬКО на РУССКОМ языке и возвращать ТОЛЬКО JSON!
...
- Не используй markdown formatting (не оборачивай JSON в блоки кода)
